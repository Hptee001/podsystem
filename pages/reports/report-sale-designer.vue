        ...
        <template #row-details="row">
            <b-card>
                <b-table v-if="!loading" ref="table" :fields="fields" :items="row.item.details" :tbody-tr-class="rowClass">
                    <template #cell(mockup)="row">
                       <b-img :src="row.item.mockup" height="50"></b-img>
                    </template>
                </b-table>
            </b-card>
        </template>
    ...
    data() {
        return {
            ...
            fields: [{
                    label: 'mockup',
                    key: 'mockup',
                    thClass: 'mytbthumbnail',
                    tdClass: 'mytbthumbnail',
                    sortable: true
                }, {
                    label: 'name',
                    key: 'name',
        ...
        loadReport(fromdate, todate) {
            ...
            this.$axios.get("/reports/generalorders/productsdesigner?fromdate=" + this.input.fromdate + "&todate=" + this.input.todate, {
                ...
                .then((response) => {
                    ...
                    this.itemsgroup.sort((a,b)=>b.qty-a.qty)
                    })
                .catch((error) => {
                    console.log(error);
                    this.loading = false;
