    ...
    <b-container class="mb-container">
        ...
        <b-row class="r-item mt-3" v-for="(item, index) in order.items" :key="item.id">
            ...
            <b-col v-show="enableFulfillment" v-if="item.name != 'Fast Shipping'">
                <b-form-select :disabled="!order.fulfillment_order_id==''" v-model="item.style_id" @input="inputBlueprint(item.style_id, index)">
                ...
                <b-form-select :disabled="!order.fulfillment_order_id==''" v-model="item.provider_id" class="mt-3" @input="inputProvider(item.style_id, item.provider_id, index)">
                ...
                <b-input-group prepend="Qty" class="mt-3">
                    ...
                    <b-form-select :disabled="!order.fulfillment_order_id==''" v-model="item.variant_id" @input="inputVariant(item)">
...
export default {
    ...
    methods: {
        ...
        async inputVariant(item) {
           // console.log(item.fulfillment_qty)
            const json = JSON.stringify(item);
